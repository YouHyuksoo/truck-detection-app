...
export function StatusMonitor({ device }: StatusMonitorProps) {
  const [status, setStatus] = useState(device.status)
  const [lastConnected, setLastConnected] = useState<Date | undefined>(device.lastConnected)
  ...
  useEffect(() => {
    ...
    setLastConnected(device.lastConnected ? new Date(device.lastConnected) : undefined)
  }, [device])

  // 주기적으로 PLC 상태 업데이트 (5초마다)
  useEffect(() => {
      ...
      try {
        ...
        if (status === ConnectionStatus.CONNECTED) {
          ...
          if (Math.random() > 0.1) {
            ...
            toast({
              title: "연결 끊김 감지",
              description: "PLC와의 연결이 끊어졌습니다. 다시 연결을 시도하세요.",
              variant: "destructive",
            })
          }
        }
        ...
        <div className="space-y-2">
          ...
          {lastConnected ? (
            <div className="text-sm">
              {lastConnected.toLocaleDateString()} {lastConnected.toLocaleTimeString()}
            </div>
          ) : (
